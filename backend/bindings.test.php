...
/**
 * Projects module — TEST DI bindings (KernelHarness).
 *
 * Mirrors backend/bindings.php exactly EXCEPT for the 3 repository bindings,
 * which substitute InMemory fakes from DaemsModule\Projects\Tests\Support.
 *
 * Inventory: 3 InMemory repos + 13 public + 12 admin use cases + 2 controllers = 30 bindings.
 */
return static function (Container $container): void {
    // ---------------------------------------------------------------------
    // 3× Repository bindings — InMemory fakes
    // ---------------------------------------------------------------------
    $container->singleton(
        ProjectRepositoryInterface::class,
        static fn() => new InMemoryProjectRepository(),
    );
    $container->singleton(
        ProjectCommentModerationAuditRepositoryInterface::class,
        static fn() => new InMemoryProjectCommentModerationAuditRepository(),
    );
    $container->singleton(
        ProjectProposalRepositoryInterface::class,
        static fn() => new InMemoryProjectProposalRepository(),
    );
